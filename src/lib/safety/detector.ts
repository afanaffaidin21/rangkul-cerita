...
const IMMINENT_PATTERNS = [
  ...
  /(?:malam ini|sekarang|sebentar lagi|dalam waktu dekat)/i,
...
function detectContext(text: string): SafetyContext {
  if (QUOTED_CONTEXT.test(text)) return "QUOTED";
  if (THIRD_PERSON_REFERENCES.test(text) && !SELF_REFERENCES.test(text)) return "THIRD_PERSON";
  if (SELF_REFERENCES.test(text)) return "SELF";
  return "UNKNOWN";
}
...
function classifyLevel(
  ...
  const negated = NEGATION.test(normalized)
    && !/(?:gak|tidak|nggak)\s+kuat/i.test(normalized)
    && !/(?:gak|nggak|tidak)\s+mau hidup lagi/i.test(normalized)
    || /bukan karena ingin mati/i.test(normalized);
  ...
  if (contextualSelf && !negated && high) return SAFETY_RISK_LEVELS.HIGH;
...
export function classifySafetyRisk(text: string): SafetyClassification {
  ...
  if (NEGATION.test(normalized)) addSignal(signals, "NEGATION");

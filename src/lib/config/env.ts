...
const runtimeEnvSchema = z.object({
  ...
  RATE_LIMIT_UNSUBSCRIBE_MAX: z.string().trim().optional(),
  RATE_LIMIT_UNSUBSCRIBE_WINDOW_SECONDS: z.string().trim().optional(),
...
export function getRuntimeEnv(env: NodeJS.ProcessEnv = process.env): RuntimeEnv {
  const parsed = runtimeEnvSchema.safeParse({
    ...
    RATE_LIMIT_UNSUBSCRIBE_MAX: env.RATE_LIMIT_UNSUBSCRIBE_MAX,
    RATE_LIMIT_UNSUBSCRIBE_WINDOW_SECONDS: env.RATE_LIMIT_UNSUBSCRIBE_WINDOW_SECONDS,
